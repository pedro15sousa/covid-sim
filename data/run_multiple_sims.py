...
def parse_args():
    ...
    parser.add_argument(
            "--param_values",
            help="List of parameter values to run simulations with",
            default=None,
            nargs='+'
    )
...
def update_param_file(param_file, param_value):
    with open(param_file, "r") as file:
        lines = file.readlines()

    # Find the line containing the parameter you want to update
    param_name = "Relative household contact rate after closure"  # Replace with the actual parameter name
    param_line = next((line for line in lines if line.startswith("[" + param_name + "]")), None)

    if param_line:
        param_index = lines.index(param_line)
        lines[param_index + 1] = str(param_value) + "\n"

        with open(param_file, "w") as file:
            file.writelines(lines)
    else:
        print("Parameter not found in the file.")

args.param_values = [0.1]
for param_value in args.param_values:
    cf = os.path.join(args.paramdir, "p_{0}.txt".format(control_roots[0]))
